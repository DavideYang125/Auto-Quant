...
def filter_2023(df):
    """筛选2023年数据"""
    df = df[df['datetime'] >= '2023-01-01'].copy()
...
def main():
    ...
    # 总结
    print(f"\n{'='*70}")
    print("SUMMARY - 2023 Backtest Results")
    print(f"{'='*70}")
    ...
    print(f"\n{'='*70}")
    print("2023 vs 2024 Comparison:")
    print(f"{'='*70}")

    print("\n2023 Results:")
    for r in results:
        print(f"  {r['pair']}: {r['strategy_return']:+.2f}% vs Benchmark {r['benchmark_return']:+.2f}% ({r['excess_return']:+.2f}%)")

    print("\n2024 Results (from previous test):")
    print("  BTC_USDT: +161.78% vs Benchmark +120.31% (+35.47%)")
    print("  ETH_USDT: +95.80% vs Benchmark +45.40% (+44.40%)")
